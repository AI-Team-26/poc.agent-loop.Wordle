...
- C# console app, .NET 8. Layout: `src/WordleJudge/` (app), `tests/WordleJudge.Tests/` (NUnit + Unquote).
...
## Steps
- [ ] **Step 1 — Scaffold.** Create solution + both projects per layout above, wire central package management, trivial `Program.cs` printing "hello", one smoke test. Create `.gitignore` covering at least: `bin/`, `obj/`, `agent_build/`, `loop.log`, `.LOOP_STOP`. Create `verify.sh` containing only: `set -e` + `dotnet build -o agent_build`. Run `./verify.sh` until green.
- [ ] **Step 2 — Core Judge() + unit tests.** Implement pure `Judge(answer, guess)` with correct duplicate-letter handling. NUnit + Unquote tests covering all-G, all-dot, basic Y, plus edge cases computed BY HAND before writing assertions: `BOOKS`/`TOOTS` = `.GG.S`, `AAAAA`/`AAAAB` = `GGGG.`, and one case where the guess has MORE of a letter than the answer does. Append to `verify.sh`: `dotnet test -o agent_build`.
